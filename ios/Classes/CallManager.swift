...
class CallManager: NSObject {
    ...
    func startCall(_ data: Data) {
        let handle = CXHandle(
            type: getHandleType(data.handleType),
            value: data.getEncryptHandle()
        )
        ...
        requestTransaction(callTransaction, action: "startCall", completion: { _ in
    ...
    func muteCall(call: Call, isMuted: Bool) {
        ...
        requestTransaction(callTransaction, action: "muteCall")
    }
    
    func holdCall(call: Call, onHold: Bool) {
        let holdAction = CXSetHeldCallAction(call: call.uuid, onHold: onHold)
        let callTransaction = CXTransaction()
    ...
    func endCall(call: Call) {
        ...
        requestTransaction(callTransaction, action: "endCall")
    }
    ...
    func endCallAlls() {
        ...
        for call in calls {
            ...
            let callTransaction = CXTransaction()
            callTransaction.addAction(endCallAction)
            requestTransaction(callTransaction, action: "endCallAlls")
        }
    }
    ...
    func callWithUUID(uuid: UUID) -> Call? {
        guard let idx = calls.firstIndex(where: { $0.uuid == uuid })
        else { return nil }
    ...
    private func postCallNotification() {
        NotificationCenter.default.post(
            name: type(of: self).callsChangedNotification,
            object: self
        )
    }
}
